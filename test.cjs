...
for (const entry of ['server/local.cjs', 'tests/vercel-host.mjs']) test(entry + ': Oturum, yetki, formlar, görüşmeler, Excel ve kalıcılık',async()=>{
  ...
  try{
    ...
    const tugbaData=(await request('/api/data?schoolId=nazmi','GET',null,tugba)).data;assert.deepEqual(tugbaData.classes.map(c=>c.name),['Anaokulu 3 Yaş','Anaokulu 4 Yaş','Anaokulu 5 Yaş A','Anaokulu 5 Yaş B']);assert.equal(tugbaData.students.length,45);
    ...
    assert.equal(d.students.filter(student=>student.name==='ASLAN OĞUZ'&&student.classId==='nazmi-anaokulu4yas').length,1);
