...
for (const entry of ['server/local.cjs', 'tests/vercel-host.mjs']) test(entry + ': Oturum, yetki, formlar, görüşmeler, Excel ve kalıcılık',async()=>{
  ...
  try{
    ...
    assert.equal(d.school.id,'nazmi');assert.equal(d.classes.length,13);assert.equal(d.students.length,216);
    ...
    const tugbaData=(await request('/api/data?schoolId=nazmi','GET',null,tugba)).data;assert.deepEqual(tugbaData.classes.map(c=>c.name),['Anaokulu 3 Yaş','Anaokulu 4 Yaş','Anaokulu 5 Yaş A','Anaokulu 5 Yaş B']);assert.equal(tugbaData.students.length,44);
    ...
    const expectedClassCounts={'Anaokulu 3 Yaş':11,'Anaokulu 4 Yaş':10,'Anaokulu 5 Yaş A':11,'Anaokulu 5 Yaş B':12,'1-A':17,'1-B':16,'2-A':20,'2-B':18,'3-A':21,'3-B':20,'3-C':20,'4-A':20,'4-B':20};
    ...
    assert.equal(d.students.filter(student=>student.name==='GİRAY DORUK ONGUN'&&student.classId==='nazmi-anaokulu5yasa').length,1);
