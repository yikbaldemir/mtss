...
for (const entry of ['server/local.cjs', 'tests/vercel-host.mjs']) test(entry + ': Oturum, yetki, formlar, görüşmeler, Excel ve kalıcılık',async()=>{
  ...
  try{
    ...
    assert.equal(d.school.id,'nazmi');assert.equal(d.classes.length,13);assert.equal(d.students.length,208);
    assert.equal(atagen.school.id,'atagen');assert.equal(atagen.classes.length,8);assert.equal(atagen.students.length,92);
    ...
    const tugbaData=(await request('/api/data?schoolId=nazmi','GET',null,tugba)).data;assert.deepEqual(tugbaData.classes.map(c=>c.name),['Anaokulu 3 Yaş','Anaokulu 4 Yaş','Anaokulu 5 Yaş A','Anaokulu 5 Yaş B']);assert.equal(tugbaData.students.length,37);
    const kubraData=(await request('/api/data?schoolId=nazmi','GET',null,kubra)).data;assert.deepEqual(kubraData.classes.map(c=>c.name),['1-A','1-B','2-A','2-B']);assert.equal(kubraData.students.length,70);
    ...
    const expectedClassCounts={'Anaokulu 3 Yaş':7,'Anaokulu 4 Yaş':10,'Anaokulu 5 Yaş A':10,'Anaokulu 5 Yaş B':10,'1-A':16,'1-B':16,'2-A':20,'2-B':18,'3-A':21,'3-B':20,'3-C':20,'4-A':20,'4-B':20};
    for(const c of d.classes)assert.equal(d.students.filter(s=>s.classId===c.id).length,expectedClassCounts[c.name]);
    const expectedAtagenClassCounts={'Anaokulu 3 Yaş':5,'Anaokulu 4 Yaş':10,'Anaokulu 5 Yaş':11,'1-A':13,'2-A':12,'3-A':11,'4-A':15,'4-B':15};
