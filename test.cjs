...
for (const entry of ['server/local.cjs', 'tests/vercel-host.mjs']) test(entry + ': Oturum, yetki, formlar, görüşmeler, Excel ve kalıcılık',async()=>{
  ...
  try{
    ...
    assert.equal(d.school.id,'nazmi');assert.equal(d.classes.length,13);assert.equal(d.students.length,209);
    ...
    const expectedClassCounts={'Anaokulu 3 Yaş':7,'Anaokulu 4 Yaş':10,'Anaokulu 5 Yaş A':10,'Anaokulu 5 Yaş B':10,'1-A':17,'1-B':16,'2-A':20,'2-B':18,'3-A':21,'3-B':20,'3-C':20,'4-A':20,'4-B':20};
