...
const classes = definitions.map(item => {
  ...
  return {
    ...
    legacyId,
    students,
    studentIds: students.map(name => {
      const identityClass = historicalStudentClasses[`${item.schoolId}\0${name}`] || item.name;
      return `${item.schoolId}-${crypto.createHash('sha256').update(`${item.schoolId}-${slug(identityClass)}\0${name}`).digest('hex').slice(0, 16)}`;
    })
  };
});
